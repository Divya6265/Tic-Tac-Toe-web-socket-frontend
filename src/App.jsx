...
function App() {
  ...
  const [isSocketConnected, setIsSocketConnected] = useState(false); // Track socket connection state
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('connect', () => {
        setIsSocketConnected(true); // Set socket connection state to true
        setPlayOnline(true);
      });
  ...
  const handlePlay = async () => {
    ...
    if (result.isConfirmed) {
      ...
      newSocket.on('connect_error', () => {
        setIsSocketConnected(false); // Set socket connection state to false
      });
    ...
    <div className='bg-[#4b495f] text-white w-full h-screen'>
      {!playOnline && (
        <div className='flex justify-center items-center h-full'>
          <button
            onClick={handlePlay}
            className='bg-[#e4ca56] border-0 font-semibold text-5xl text-black py-3 px-2 rounded cursor-pointer'
          >
            Play Online
          </button>
        </div>
      ...
      {playOnline && !isSocketConnected && (
        <div className='flex justify-center items-center h-full'>
          ...
          <p className='text-3xl font-semibold'>
            Socket connection is yet to be established. Please wait. This is due
            to Vercel server priority. If possible, please download the code from
            GitHub and run it on your local machine.
          </p>
        </div>
      )}
      ...
      {playOnline && isSocketConnected && opponent !== null && opponentLeft && (
        <div className='flex flex-col justify-center items-center h-full'>
          ...
          <button
            onClick={handleAgain}
            className='bg-[#e4ca56] border-0 font-semibold text-xl text-black mt-3 py-1 px-2 rounded cursor-pointer'
          >
            Play Again
          </button>
        </div>
      ...
      {playOnline && isSocketConnected && opponent !== null && !opponentLeft && (
        <div className='w-fit gap-5 m-auto flex flex-col pt-30'>
          <div className='flex justify-between w-120'>
            <p
              className={`h-12 w-45 rounded-bl-[50px] pt-3 pb-3 text-center rounded-tr-[50px] font-semibold ${
                !finishedState && currentPlayer === PlayerName
                  ? 'bg-[#DD7F9F]'
                  : 'bg-[#62607b]'
              }`}
            >
            ...
            <p
              className={`h-12 w-45 rounded-bl-[50px] pt-3 pb-3 text-center rounded-tr-[50px] font-semibold ${
                !finishedState && currentPlayer === opponent
                  ? 'bg-[#3FA7F0]'
                  : 'bg-[#62607b]'
              }`}
            >
          ...
          <div className='m-auto felx flex-col justify-center items-center'>
            <h1 className='text-4xl text-center rounded bg-[#62607b] py-3 m-auto px-12 font-semibold w-80'>
              Tic Tac Toe
            </h1>
            ...
            <div className='w-100 flex flex-col items-center'>
              ...
              {finishedArray && winner && (
                <button
                  ...
                  className='bg-[#e4ca56] border-0 font-semibold text-xl text-black mt-3 py-1 px-2 w-fit rounded cursor-pointer'
                >
                  Want a Rematch
                </button>
              )}
              ...
              {!finishedState && opponent && (
                <p className='text-2xl font-semibold text-center mt-4'>
                  You are playing against {opponent}
                </p>
              )}
            </div>
          </div>
